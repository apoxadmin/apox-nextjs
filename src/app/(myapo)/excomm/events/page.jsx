		...
		<div>
			<div className="flex space-x-4">
				...
				{!event.reviewed ? (
					<button
						onClick={() => {
							if (approveEvent(event?.id)) setTimeout(getEvents, 250);
						}}>
						Approve
					</button>
				) : (
					<button
						onClick={() => {
							if (unapproveEvent(event?.id)) setTimeout(getEvents, 250);
						}}>
						Unapprove
					</button>
				)}
...
export default function ExCommEvents() {
	...
	async function getEvents() {
		const eventsRequest = await supabase.from("events").select("*, event_types(*)").gte("date", startOfToday().toISOString());
		if (eventsRequest.data) setEvents(eventsRequest.data);
	}
		...
		<div className="flex flex-col items-center">
			...
			<div className="flex flex-col items-center space-y-4">
				<div className="flex flex-col items-center">
					...
					<div>
						{events.map((event, i) => {
							if (!event.reviewed) return <EventModal event={event} getEvents={getEvents} key={i} />;
