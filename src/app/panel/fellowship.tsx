...
export default function FellowshipPage() {
    ...
    React.useEffect(() => {
        async function fetchEvents() {
            const unreviewed = (await supabase.from('events').select('*, chair_joins ( users ( * ) ), event_user_joins ( users ( * ) ), event_types!inner(*)').eq('event_types.name', 'fellowship').eq('reviewed', false)).data;
            setUnreviewedEvents(unreviewed);
            const upcoming = (await supabase.from('events').select('*, chair_joins ( users ( * ) ), event_user_joins ( users ( * ) ), event_types!inner(*)').eq('event_types.name', 'fellowship').eq('reviewed', true).gte('startDate', startOfToday().toISOString())).data;
    ...
    return (
        <div className="flex flex-col items-center space-y-4 pb-4 px-4">
            ...
            />
            <h1 className="text-center text-xl pt-4 w-full font-medium underline">Upcoming Fellowships</h1>
            <div className="flex flex-col space-y-2 w-full max-w-sm">
            {
