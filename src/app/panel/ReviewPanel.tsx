...
export default function ReviewPanel({ eventTypes }: { eventTypes: Array<string> }) {
    ...
    React.useEffect(() => {
        async function fetchEvents() {
            const unreviewed = (await supabase.from('events').select('*, chair_joins ( users ( * ) ), event_user_joins ( users ( * ) ), event_types!inner(*)').in('event_types.name', eventTypes).eq('reviewed', false)).data;
            setUnreviewedEvents(unreviewed);
            const upcoming = (await supabase.from('events').select('*, chair_joins ( users ( * ) ), event_user_joins ( users ( * ) ), event_types!inner(*)').in('event_types.name', eventTypes).eq('reviewed', true).gte('startDate', startOfToday().toISOString())).data;
